  ...
  try {
    ...
    if (user || doctor || nurse) {
      ...
      if (isPasswordValid) {
        ...
        if (user) {
          token = jwt.sign({ id: user._id, role: user.role }, appConfig.jwt.secret, {
            expiresIn: "2d",
          });
          role = user.role;
          loggedInUser = user;
        } else if (doctor) {
          token = jwt.sign({ id: doctor._id, role: doctor.role }, appConfig.jwt.secret, {
            expiresIn: "2d",
          });
          role = doctor.role;
          loggedInUser = doctor;
        } else if (nurse) {
          token = jwt.sign({ id: nurse._id, role: nurse.role }, appConfig.jwt.secret, {
            expiresIn: "2d",
          });
          role = nurse.role;
          loggedInUser = nurse;
